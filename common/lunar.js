;(function(root,factory){
  ...
  var HolidayUtil = (function(_NAMES){
    ...
    // _DATA是一个包含假期数据的字符串，每个假期的日期和其他信息都被编码在其中。
    ...
    // _padding函数接收一个数字 n，如果 n 小于10，则在其前面添加一个 '0'，然后返回结果。
    var _padding = function(n){
      return (n<10?'0':'')+n;
    };
    // _ymd函数接收一个字符串 s，检查该字符串中是否包含 '-'，如果不包含，则将其转换为日期格式 'YYYY-MM-DD'，否则直接返回字符串。
    // 年月日 year month day
    var _ymd = function(s){
      return s.indexOf('-')<0?(s.substr(0,4)+'-'+s.substr(4,2)+'-'+s.substr(6)):s;
    };
// 代码定义了 _buildHoliday 函数，用于构建假期对象。该函数接收 day、name、work 和 target 参数，并返回一个包含 getter 和 setter 方法的假期对象。假期对象有以下属性和方法：
  // _p 是一个包含假期对象属性的对象，其中包括 day、name、work 和 target。
  // getDay 和 setDay 方法用于获取和设置假期的日期。
  // getName 和 setName 方法用于获取和设置假期的名称。
  // isWork 和 setWork 方法用于获取和设置假期是否需要调休。
  // getTarget 和 setTarget 方法用于获取和设置调休的目标日期。
  // toString 方法返回假期对象的字符串表示形式。
...
// 代码提供了三个主要的函数：
// getHoliday：接受一个或三个参数，并根据给定的参数返回一个单独的节假日对象。该函数内部调用了 _findHolidaysForward。
// getHolidays：接受一个或两个参数，并根据给定的参数返回一个节假日对象的数组。该函数内部调用了 _findHolidaysForward。
// getHolidaysByTarget：接受一个或三个参数，并根据给定的参数返回一个节假日对象的数组。该函数内部调用了 _findHolidaysBackward。

    // 定义了两个辅助函数：_buildHolidayForward 和 _buildHolidayBackward。这些函数解析表示节假日对象的字符串，并使用 _buildHoliday 函数创建相应的节假日对象。
    ...
    // _findForward 和 _findBackward。这些函数在一个较大的字符串 _DATA_IN_USE 中搜索给定的关键字。_findForward 从数据的开头开始搜索关键字，而 _findBackward 从数据的末尾开始搜索关键字
    ...
    // _findHolidaysForward 和 _findHolidaysBackward 利用前面的辅助函数从 _DATA_IN_USE 字符串中找到并解析节假日对象
